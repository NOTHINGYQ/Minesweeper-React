...
import generateCell from "../../util/generateCell";
import setCell from "../../util/setCell";

const Main = () => {
    const [cells, setCells] = useState(generateCell());
    ...
    const handleButtonClick = (rowParam, colParam) => e => {
        ...
        if (cell.value === -1) {
            setHasLost(true);
            let newCells = setCell(gameCells, rowParam, colParam, "red", true);
        ...
        // display number
        if (cell.value > 0) {
            gameCells = setCell(gameCells, rowParam, colParam, "state", 1);
        }
    ...
    const handleButtonContextMenu = (rowParam, colParam) => e => {
        ...
        if (cell.state === 0) {
            let newCells = setCell(cells, rowParam, colParam, "state", 2);
            setCells(newCells);
        ...
        const newCells = setCell(cells, rowParam, colParam, "state", 0);
        setCells(newCells);
        setMineCounter(mineCounter + 1);
    };
    ...
    const openMultiple = (cellsParam, rowParam, colParam) => {
        ...
        let newCells = setCell(cellsParam, rowParam, colParam, "state", 1);
        ...
        if (topLeftCell && topLeftCell.state === 0 && topLeftCell.value === 0) {
            ...
            newCells = setCell(newCells, rowParam - 1, colParam - 1, "state", 1);
        }
        ...
        if (topCell && topCell.state === 0 && topCell.value === 0) {
            ...
            newCells = setCell(newCells, rowParam - 1, colParam, "state", 1);
        }
        ...
        if (leftCell && leftCell.state === 0 && leftCell.value === 0) {
            ...
            newCells = setCell(newCells, rowParam, colParam - 1, "state", 1);
        }
